...
$related = Berita::where('id', '!=', $berita->id)->where('kategori', $berita->kategori)->orderBy('published_at', 'desc')->limit(3)->get();
...
return ['id' => $l->id,'judul' => $l->nama,'nama' => $l->nama,'deskripsi' => $l->deskripsi,'icon' => $l->ikon,'ikon' => $l->ikon,'url' => $l->link,'link' => $l->link,'warna' => $l->warna,'kategori' => $l->kategori,'urutan' => $l->urutan,'aktif' => $l->aktif,'is_active' => (bool)$l->aktif,];
});
...
$request->validate(['nama' => 'required|string|max:255','email' => 'required|email','telepon' => 'nullable|string','subjek' => 'required|string|max:255','pesan' => 'required|string',]);
...
$items[] = ['id' => md5((string)$item->link),'judul' => $title,'status' => $status,'tanggal' => date('d M Y', strtotime((string)$item->pubDate)),'link' => (string)$item->link];
$count++;
}
}
...
return [['id' => '1', 'judul' => '[HOAKS] Bantuan Sosial Tunai Rp2 Juta dari Pemkab Sanggau', 'status' => 'HOAKS', 'tanggal' => '10 Mei 2026', 'link' => '#'],['id' => '2', 'judul' => '[DISINFORMASI] Pemadaman Listrik Total Selama Seminggu', 'status' => 'DISINFORMASI', 'tanggal' => '08 Mei 2026', 'link' => '#'],['id' => '3', 'judul' => '[HOAKS] Pendaftaran CPNS Jalur Khusus Tanpa Tes', 'status' => 'HOAKS', 'tanggal' => '05 Mei 2026', 'link' => '#']];
}
}
